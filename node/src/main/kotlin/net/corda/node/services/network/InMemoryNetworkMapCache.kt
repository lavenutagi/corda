...
open class InMemoryNetworkMapCache(private val serviceHub: ServiceHubInternal?) : SingletonSerializeAsToken(), NetworkMapCacheInternal {
    ...
    override fun getPartyInfo(party: Party): PartyInfo? {
        val node = registeredNodes[party.owningKey] //TODO Query by legalIdentity
        if (node != null) {
    ...
    override fun getNodeByLegalIdentityKey(identityKey: PublicKey): NodeInfo? = registeredNodes[identityKey] //TODO Query by legalIdentity
    ...
    fun queryByLegalIdentity(identityKey: PublicKey): List<NodeInfo> {
        val session = createSession()
        return session?.use {
    ...
    fun queryByName(name: X500Name): List<NodeInfo> {
        val session = createSession()
        return session?.use {
    ...
    fun queryByAddress(hostAndPort: NetworkHostAndPort): NodeInfo? {
        val session = createSession()
        return session?.use {
